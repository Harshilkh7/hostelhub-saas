...
async function createTokenPair(user) {
  ...
  await pool.query(
    `INSERT INTO auth_sessions(id,user_id,refresh_jti_hash,refresh_expires_at) VALUES($1,$2,$3,NOW()+INTERVAL '7 days')`,
    [sessionId, user.id, hash(refreshJti)]
  );
...
async function rotateRefreshToken(token) {
  ...
  try {
    ...
    const sessionResult = await client.query(
      `SELECT id FROM auth_sessions WHERE refresh_jti_hash=$1 AND user_id=$2 AND revoked_at IS NULL AND refresh_expires_at > NOW() FOR UPDATE`,
      [hash(decoded.jti), decoded.userId]
    );
    ...
    return { user, accessToken: nextAccess, refreshToken: nextRefresh };
  } catch (error) {
...
async function revokeSessionByRefreshToken(token) {
  try {
    const decoded = jwt.verify(token, refreshSecret());
    if (decoded.jti) await pool.query(`UPDATE auth_sessions SET revoked_at=NOW() WHERE refresh_jti_hash=$1 AND revoked_at IS NULL`, [hash(decoded.jti)]);
  } catch (_) {}
}
